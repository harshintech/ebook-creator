...
const updateBookCover = async (req, res) => {
  try {
    ...
    const upadatedBook = await book.save();
    res.status(200).json(upadatedBook);
  } catch (error) {
    res.status(500).json({ message: "Server error" });
  }
};

// @desc    Get public book details (only if status is published)
...
const getPublicBookById = async (req, res) => {
  try {
    ...
    if (book.status !== "published") {
      return res
        .status(403)
        .json({ message: "This book is not published" });
    }
