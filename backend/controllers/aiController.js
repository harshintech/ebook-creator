...
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KET });
...
const generateOutline = async (req, res) => {
  try {
    ...
    const prompt = `You are an expert book outline generator, Create a comprehensive book outline based on the following requirements:
      ...
      4. Ensure chapter build upon each other coherently
    ...
    try {
      const outline = JSON.parse(jsonString); // string --> json
...
// @desc    Generate a content for a chapter
