...
//Typography configuration mathching the pdf export
const DOCX_STYLES = {
  fonts: {
    body: "Charter",
    heading: "Inter",
...
const renderInlineTokens = (doc, tokens, options = {}) => {
  ...
  // Final flush and line termination closure handling
  if (textBuffer) {
    doc.font(currentFont).text(textBuffer, {
      ...baseOptions,
      continued: false,
...
const renderMarkdown = (doc, markdown) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      ...
      // 6. CODE BLOCK AND FENCE PROCESSING
      else if (token.type === "code_block" || token.type === "fence") {
        doc.moveDown(
          TYPOGRAPHY.spacing.paragraphSpacing / TYPOGRAPHY.sizes.body,
        );
      ...
      // 7. HORIZONTAL RULE (THEMATIC BREAK) PROCESSING
      else if (token.type === "hr") {
        doc.moveDown();
        const y = doc.y;
